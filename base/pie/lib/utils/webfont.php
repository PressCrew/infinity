<?php
/**
 * PIE API: web fonts helper class file
 *
 ...
 */
class Pie_Easy_Webfont extends Pie_Easy_Base
{
	/**
	 * Singleton instance
	 * 
	 * @var Pie_Easy_Webfont
	 */
	...
	private function __construct( $max_age = 86400 )
	{
		...
		$this->services = new Pie_Easy_Map();

		// add google always (for now)
		$this->add_google();

		// set up export
		$this->export = new Pie_Easy_Export( 'webfont', 'json', array($this,'update') );
	 ...
	 * 
	 * @param integer $max_age
	 * @return Pie_Easy_Webfont
	 */
	public function instance( $max_age = 86400 )
	{
		if ( !self::$instance instanceof Pie_Easy_Webfont ) {
			self::$instance = new Pie_Easy_Webfont( $max_age );
		}
	...
	public function add_google()
	{
		...
		if ( !$this->services->contains( 'google' ) ) {
			// create new instance
			$google = new Pie_Easy_Webfont_Service_Google();
 ...
 * @subpackage utils
 */
abstract class Pie_Easy_Webfont_Service extends Pie_Easy_Base
{
	// variants
	const VARIANT_N1 = 'n1';
	const VARIANT_N2 = 'n2';
 ...
 * @subpackage utils
 */
class Pie_Easy_Webfont_Service_Google extends Pie_Easy_Webfont_Service
{
	/**
