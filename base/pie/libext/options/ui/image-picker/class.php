<?php
/**
 * PIE API: option extensions, UI image picker class file
 *
 ...
 */
class Pie_Easy_Exts_Options_Ui_Image_Picker
	extends Pie_Easy_Exts_Options_Ui_Scroll_Picker
		implements Pie_Easy_Options_Option_Static_Image, Pie_Easy_Options_Option_Auto_Field
{
	/**
	 */
	...
	{		
		...
		if ( strlen( $this->file_extension ) ) {
			...
			if ( $this->file_directory ) {
				// locate it
				$path = Pie_Easy_Scheme::instance()->locate_file( $this->file_directory );
				// find it?
				if ( $path ) {
					// found the path, list files with extension
					$images = Pie_Easy_Files::list_filtered( $path, sprintf( '/\.%s$/', $this->file_extension ), true );
					// find any images?
					if ( count( $images ) ) {
						// options to return
						$field_options = array();
						...
						foreach ( $images as $key => $value ) {
							...
							$field_options[$key] = Pie_Easy_Files::theme_file_to_url($value);
						}
